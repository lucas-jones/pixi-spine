import { Sprite, Container, utils, DRAW_MODES, Polygon, Texture, Rectangle, Graphics, SimpleMesh } from "pixi.js";
...
export class Spine extends Container {
    ...
    update(dt: number) {
        ...
        for (let i = 0, n = slots.length; i < n; i++) {
            ...
            if (attachment instanceof RegionAttachment) {
                let region = (attachment as RegionAttachment).region;
                if (region) {
                    if (slot.currentMesh) {
                ...
                (attachment as VertexAttachment).computeWorldVerticesOld(slot, slot.currentMesh.vertices);
        ...
        let clippingAttachment: ClippingAttachment = null;
    ...
    private setSpriteRegion(attachment: RegionAttachment, sprite: SpineSprite, region: TextureRegion) {
    ...
    private setMeshRegion(attachment: MeshAttachment, mesh: SpineMesh, region: TextureRegion) {
        mesh.region = region;
     ...
     * @private
     */
    createSprite(slot: Slot, attachment: RegionAttachment, defName: string) {
     ...
     * @private
     */
    createMesh(slot: Slot, attachment: MeshAttachment) {
        let region = attachment.region;
        if (slot.tempAttachment === attachment) {
            region = slot.tempRegion;
            slot.tempAttachment = null;
    ...
    createGraphics(slot: Slot, clip: ClippingAttachment) {
    ...
    updateGraphics(slot: Slot, clip: ClippingAttachment) {
